...
class PositionVisitor {
    ...
    visit(node, options, segments) {
        ...
        switch (node.type) {
            ...
            case odata_annotation_core_1.ATTRIBUTE_TYPE:
     ...
     * @private
     */
    visitText(node, options, segments) {
        const { position } = options;
        const nodeRange = node.range ?? undefined;
        if ((0, odata_annotation_core_1.positionContained)(nodeRange, position)) {
            return {
                path: [...segments, 'text'],
                range: nodeRange
            };
     ...
     * @private
     */
    visitAttribute(attribute, options, segments) {
        const { position } = options;
        const nameRange = attribute.nameRange ?? undefined;
        if ((0, odata_annotation_core_1.positionContained)(nameRange, position)) {
            return {
                path: [...segments, 'name'],
                range: nameRange
            };
        ...
        if ((0, odata_annotation_core_1.positionContained)(valueRange, position)) {
            return {
                path: [...segments, 'value'],
                range: valueRange
            };
     ...
     * @private
     */
    visitElement(element, options, segments) {
        const { position } = options;
        const range = element.range;
        if (range && (0, odata_annotation_core_1.positionContained)(range, position)) {
            const attributeNames = Object.keys(element.attributes);
            if ((0, odata_annotation_core_1.positionContained)(element.nameRange, position)) {
                return {
                    path: [...segments, 'name'],
            ...
            if ((0, odata_annotation_core_1.positionContained)(element.contentRange, position)) {
     ...
     * @private
     */
    visitContent(element, options, segments) {
        const { position } = options;
        ...
        const index = element.content.findIndex((item) => item.range && (0, odata_annotation_core_1.isBefore)(position, item.range.start, true));
        return {
 ...
 */
function findNode(assignment, annotations, position) {
    const result = visitor.visit(annotations, { position }, ['']);
    if (result) {
        const path = (0, cds_annotation_parser_1.findAnnotationNode)(assignment, { position, includeDelimiterCharacters: false });
 ...
 */
function getTermNodes(terms, path) {
    // ignore the first empty segment
    const [, indexString, ...segments] = path;
    const index = parseInt(indexString, 10); // or use Number(indexString);
    let node = terms[index];
    const nodes = [];
    for (const segment of segments) {
        if (typeof node === 'object' && node !== null) {
            node = node[segment]; // Use any to handle indexing with a string
        }
        nodes.push(node);
    }
    return nodes;
 ...
 */
function adjustReturnValue(astPath, assignment, terms, value, position) {
    const astNodes = (0, cds_annotation_parser_1.getAstNodes)(assignment, astPath);
    const node = astNodes?.[astNodes.length - 1];
    if (!node) {
        return value;
    }
    const nodes = getTermNodes(terms, value.path.filter((segment) => !segment.startsWith('$')));
    ...
    switch (node.type) {
        case cds_annotation_parser_1.RECORD_PROPERTY_TYPE:
        case cds_annotation_parser_1.ANNOTATION_TYPE:
            return beforeValue(node, value, position);
        case cds_annotation_parser_1.NUMBER_LITERAL_TYPE:
        case cds_annotation_parser_1.STRING_LITERAL_TYPE:
            return stringValue(node, value, position);
        case cds_annotation_parser_1.ENUM_TYPE:
            return enumTextNode(value);
        case cds_annotation_parser_1.EMPTY_VALUE_TYPE:
            return emptyValue(node, value);
        case cds_annotation_parser_1.PATH_TYPE:
 ...
 * @returns Returns the adjusted VisitorReturnValue.
 */
function beforeValue(node, value, position) {
    const lastPathSegment = value.path.slice(-1)[0];
    if (node.type === cds_annotation_parser_1.RECORD_PROPERTY_TYPE && node.name.value === cds_annotation_parser_1.ReservedProperties.Value) {
        // for $value pointer should not be adjusted
        return value;
    }
    if (lastPathSegment?.startsWith('$') &&
        node.colon?.range?.end &&
        !(0, odata_annotation_core_1.isBefore)(position, node.colon.range.end) &&
        node.value?.range?.start &&
        (0, odata_annotation_core_1.isBefore)(position, node.value.range.start, true)) {
 ...
 * @returns Returns the adjusted VisitorReturnValue.
 */
function stringValue(node, value, position) {
    const range = (0, cds_annotation_parser_1.nodeRange)(node, false);
    const lastPathSegment = value.path.slice(-1)[0];
 ...
 * @returns Returns the adjusted VisitorReturnValue.
 */
function emptyValue(node, value) {
    if (node.range) {
        return {
            path: value.path,
            range: node.range
 ...
 * @returns Returns the adjusted VisitorReturnValue.
 */
function enumCollection(astNodes, nodes, value, position) {
    // The end of nodes array should look like this - [..., element, content, textNode, text ]
    const [parent, , textNode] = nodes.slice(-4);
    if (parent?.type !== odata_annotation_core_1.ELEMENT_TYPE || parent.name !== "EnumMember" /* Edm.EnumMember */) {
        return value;
    }
    const collection = findLastNode(astNodes, cds_annotation_parser_1.COLLECTION_TYPE);
    if (!collection) {
        return value;
    }
    const enumNode = collection.items.find((child) => (0, odata_annotation_core_1.positionContained)(child.range, position));
    if (textNode?.type !== odata_annotation_core_1.TEXT_TYPE || enumNode?.type !== cds_annotation_parser_1.ENUM_TYPE || enumNode.range === undefined) {
        return value;
    }
 ...
 */
function findLastNode(nodes, type) {
 ...
 */
function edmJsonContent(astNodes, value, position) {
    const edmJsonIndex = astNodes.findIndex((node) => !Array.isArray(node) && node.type === cds_annotation_parser_1.RECORD_PROPERTY_TYPE && node.name.value === cds_annotation_parser_1.ReservedProperties.EdmJson);
    if (edmJsonIndex === -1) {
        return value;
    }
    if (value.path[value.path.length - 1].startsWith('$')) {
        ...
        const range = (0, cds_annotation_parser_1.nodeRange)(node, false);
        if (node.type !== cds_annotation_parser_1.RECORD_TYPE || !range) {
            return value;
        }
        ...
        if (property.range?.start && (0, odata_annotation_core_1.isBefore)(position, property.range?.start)) {
 ...
 */
function pathTextNode(node, value, position) {
    const lastPathSegment = value.path.slice(-1)[0];
    if (lastPathSegment !== 'text') {
        // we need to only update position for text nodes
        return value;
    ...
    for (let i = 0; i < node.segments.length; i++) {
        ...
        if (segment.range && (0, odata_annotation_core_1.positionContained)(segment.range, position)) {
