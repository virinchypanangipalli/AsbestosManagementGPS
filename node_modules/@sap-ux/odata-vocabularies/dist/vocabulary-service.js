...
const vocabulary_service_1 = require("./types/vocabulary-service");
...
class VocabularyService {
     ...
     */
    toAliasQualifiedName(qualifiedName, aliasInfo) {
    ...
    constructor(includeCds, includeCdsAnalytics) {
        ...
        this.cdsVocabulary = {
            namespace: vocabulary_service_1.CDS_VOCABULARY_NAMESPACE,
            alias: vocabulary_service_1.CDS_VOCABULARY_ALIAS,
            nameMap: new Map(),
        ...
        if (includeCds) {
            this.dictionary.forEach((item) => {
                ...
                if (resolvedName.namespace === vocabulary_service_1.CDS_VOCABULARY_NAMESPACE) {
                    const name = (item.kind === 'Term' && item.cdsName) ||
                        resolvedName.name
                            .replace(/([A-Z])/g, ' $1')
                            .trim()
                            .split(' ')
                            .join('.')
                            .toLowerCase();
                    const property = `${vocabulary_service_1.CDS_VOCABULARY_ALIAS}.${resolvedName.name}`;
     ...
     */
    addCdsSpecificAnnotations(vocabularyInformation) {
     ...
     */
    getDefaultAlias(namespace) {
        return resources_1.NAMESPACE_TO_ALIAS.get(namespace);
    }
    /**
    ...
    checkTermApplicability(termName, targetKinds, targetType) {
        ...
        if (!term && !this.supportedVocabularies.has(namespace)) {
            return vocabulary_service_1.TermApplicability.UnSupportedVocabulary;
        }
        else if (!term) {
            return vocabulary_service_1.TermApplicability.UnknownTerm;
        }
        else {
            ...
            if (!applicable) {
                return vocabulary_service_1.TermApplicability.TermNotApplicable;
            }
                ...
                return this.isOfType(requiredType, targetType)
                    ? vocabulary_service_1.TermApplicability.Applicable
                    : vocabulary_service_1.TermApplicability.TypeNotApplicable;
            }
            else {
                return vocabulary_service_1.TermApplicability.Applicable;
            }
        }
    }
    /**
     ...
     */
    getElementTypeDescription(element, elementType, experimentalDescription, aliasInfo) {
        const values = [];
        ...
        if (element.kind === odata_annotation_core_types_1.COMPLEX_TYPE_KIND && element.baseType) {
     ...
     */
    getDocumentation(name, propertyName, aliasInfo) {
        const values = [];
        ...
        if (element.kind === odata_annotation_core_types_1.TERM_KIND && element.baseTerm) {
        ...
        if ((element.kind === odata_annotation_core_types_1.COMPLEX_TYPE_KIND || element.kind === odata_annotation_core_types_1.PROPERTY_KIND || element.kind === odata_annotation_core_types_1.TERM_KIND) &&
            element.constraints?.applicableTerms) {
            let applicableTerms = element.constraints.applicableTerms;
            if (aliasInfo) {
                applicableTerms = applicableTerms.map((fullyQualifiedName) => this.toAliasQualifiedName(fullyQualifiedName, aliasInfo));
            }
     ...
     * @returns - values
     */
    checkExperimentalElement(element, experimentalDescription) {
        const values = [];
     ...
     * @returns - values
     */
    checkDeprecatedElement(element) {
        const values = [];
     ...
     * @returns - values
     */
    getElementKindIsProperty(element) {
        const values = [];
     ...
     * @returns - values
     */
    getElementKindIsMemberAndTerm(element, elementType, aliasInfo) {
        const values = [];
     ...
     * @returns - values
     */
    getElementIsLanguageDependent(element, languageDependentDesc) {
        const values = [];
        if ((element.kind === odata_annotation_core_types_1.TERM_KIND || element.kind === 'Property') && element.constraints?.isLanguageDependent) {
    ...
    getElementDefaultValue(element) {
        const values = [];
        if ((element.kind === odata_annotation_core_types_1.TERM_KIND || element.kind === 'Property') && element.defaultValue) {
     ...
     * @returns - values
     */
    getElementAppliesToValue(element) {
        const values = [];
        if (element.kind === odata_annotation_core_types_1.TERM_KIND && element.appliesTo && element.appliesTo?.length > 0) {
     ...
     * @returns - values
     */
    getElementRequireTypeValue(element) {
        const values = [];
     ...
     * @returns - values
     */
    getElementLongDescription(element) {
        const values = [];
        if (element.longDescription) {
            const longDescription = element.kind === 'Member' ? `**Enum Value Long Description:**` : `**Long Description:**`;
     ...
     */
    resolveDocumentationElement(name, propertyName) {
        let element;
        let elementType;
     ...
     */
    getFormattedTypeText(element, elementType, aliasInfo) {
        let sResultText = '';
        if (element.kind === odata_annotation_core_types_1.TERM_KIND || element.kind === 'Property') {
    ...
    getFormattedNullableText(object) {
        let sResultText = '';
        const isNullable = (object.kind === odata_annotation_core_types_1.TERM_KIND || object.kind === 'Property' || object.kind === odata_annotation_core_types_1.TYPE_DEFINITION_KIND) &&
            !!object.facets &&
            !!object.facets.isNullable;
        const isCollection = (object.kind === odata_annotation_core_types_1.TERM_KIND || object.kind === 'Property') && object.isCollection;
    ...
    getTerm(termName) {
        ...
        return vocabularyObject?.kind === odata_annotation_core_types_1.TERM_KIND ? vocabularyObject : undefined;
    }
    /**
    ...
    getDerivedTypeNames(typeName, includeAbstract) {
        ...
        if (type && type.kind === odata_annotation_core_types_1.COMPLEX_TYPE_KIND) {
            ...
            while (stack.length > 0) {
                ...
                const types = this.derivedTypesPerType.get(name) ?? new Map();
    ...
    getComplexType(typeName) {
        ...
        const expandedComplexType = { baseTypes: [], ...rest };
     ...
     */
    _getComplexType(name) {
        const dictionaryType = this.dictionary.get(name);
        return dictionaryType?.kind === odata_annotation_core_types_1.COMPLEX_TYPE_KIND ? dictionaryType : undefined;
    }
    /**
