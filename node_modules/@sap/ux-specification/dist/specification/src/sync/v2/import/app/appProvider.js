...
function getPagesAndConfigs(v2Pages, importParameters) {
    ...
    for (const v2PageName in v2Pages) {
        ...
        const schemaKey = Object.keys(importParameters.appSchemas).find((element) => element.includes(id)) ||
            Object.keys(importParameters.appSchemas).find((element) => element.includes(id.slice(0, -1))) ||
            Object.keys(importParameters.appSchemas).find((element) => element.includes(page.pageType));
        // convert settings to config
        if (schemaKey) {
            if (page.pageType === ux_specification_types_1.PageTypeV2.ListReport) {
                const importLRParameters = {
                    manifest: importParameters.manifest,
                    flex: importParameters.flexChanges,
                    jsonSchema: importParameters.appSchemas[schemaKey],
                    fragments: importParameters.fragments,
                    logger: importParameters.logger
                };
                page.config = (0, listReport_1.createListReportConfig)(importLRParameters);
            }
                ...
                page.config = (0, analyticalListPage_1.createAnalyticalListPageConfig)(importALPParameters);
            }
            else {
                const importOPParameters = {
                    manifest: importParameters.manifest,
                    flex: importParameters.flexChanges,
                    jsonSchema: importParameters.appSchemas[schemaKey],
                ...
                page.config = (0, objectPage_1.createObjectPageConfigV2)(importOPParameters);
            }
        }
        if (v2Page.pages) {
            getPageConfigsAndNavigations(page, v2Page, importParameters, pages);
        }
    }
    return pages;
}
...
class V2AppProvider extends appProvider_1.AppProvider {
    constructor(importParameters) {
        ...
        // Call base
        super(importParameters, pageId, getPagesAndConfigs(v2Pages, importParameters), appSettings);
    }
}
